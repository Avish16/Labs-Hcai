...
def fact_check_claim(user_claim: str):
    """Fact-checks a claim using Responses API with web_search tool and returns structured JSON."""
...
if st.button("Check Fact"):
    if not user_claim.strip():
        ...
        with st.spinner("Verifying..."):
            ...
            result_json["confidence_score"] = round(confidence, 2)
...
if st.session_state.fact_history:
    ...
    for i, entry in enumerate(reversed(st.session_state.fact_history), start=1):
        st.markdown(f"{i}. **{entry['claim']}** — {entry['verdict']} ({entry['confidence'] * 100:.0f}% confidence)")
